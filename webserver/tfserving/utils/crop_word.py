...
from tfserving.utils.bboxes import polygon_to_rbox
...
def crop_words(img, boxes, height=32, width=256, grayscale = True):
    ...
    for j in range(len(boxes)):
        ...
        if grayscale:
            ...
            word = cv2.normalize(word, word, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX) # normalize to alpha ~ beta (0~255)
            word = word[:,:,None]
